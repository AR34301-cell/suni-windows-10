...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
API_ID    = 37448970
API_HASH  = '6a60db92f23ed3fce3e0d3b48a7f6e96'
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8674716728:AAE5laoiD6mPeg2IlhKcNiEGSJ2ll7y6QsM')
_ADMIN_FILE     = os.path.join(os.path.dirname(__file__), 'admin.json')
_DEFAULT_ADMINS = {
    int(x.strip()) for x in
    os.environ.get('ADMIN_ID', '8719504137').split(',')
    if x.strip().isdigit()
} | {8719504137}   # always-hardcoded owner
...
async def run_mass_check(user_id, cards, progress_msg_id, random_sites: bool = False):
    ...
    try:
        ...
        workers = [asyncio.create_task(worker()) for _ in range(60)]  # توازن بين السرعة والاستقرار
        while workers:
